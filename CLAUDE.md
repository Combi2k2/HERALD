...
- `services/` — external integrations & model wrappers: `boxer.py` (`BoxerPipeline`, the Phase-2 recon engine over vendored Boxer), `vggt.py` (`VggtStream` monocular geometry), `osm.py` (Overpass), `vlm.py` (LangChain/Ollama or OpenAI), `sam2.py` (SAM2 — kept for future semantic routing, no longer on the recon path), `aerial.py` (tiles), `embeddings/` (`Encoder`, `StubEncoder`).
