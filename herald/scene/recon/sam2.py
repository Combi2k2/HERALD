...
class Sam2Stream:
    """Push RGB frames, receive uint16 label maps back one chunk at a time.
    ...
    def run_chunk(self, frames: Sequence) -> list[np.ndarray]:
        ...
        with torch.inference_mode():
            ...
            for out in self.model.propagate_in_video_iterator(session):
                res = self.processor.post_process_masks(
                    [out.pred_masks],
    ...
    def push(self, rgb: np.ndarray) -> list[np.ndarray]:
    ...
    def run_stream(self, frames: Iterable) -> Iterator[np.ndarray]:
        """Lazily yield one label map per frame (RGB array or path), a chunk at a time."""
        for f in frames:
            yield from self.push(as_rgb(f))
        yield from self.finish()

    def run_video(self, frames: Sequence) -> list[np.ndarray]:
        """Whole-sequence convenience: label maps for RGB arrays or image paths."""
